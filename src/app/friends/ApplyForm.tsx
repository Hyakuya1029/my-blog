...
export default function ApplyForm() {
  ...
  const valid = name.trim() && url.trim() && description.trim() && email.trim();//申请验证逻辑
    ...
    <>
      <div className="mt-10 text-center">
        <button
          ...
          className="px-6 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white text-sm font-medium transition-all hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0"
        >
          申请友链
        </button>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => { setOpen(false); reset(); }}>
          <div className="absolute inset-0 bg-black/40" />
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            <button
            ...
            {done ? (
              <div className="text-center py-8">
                <p className="text-sky-700 dark:text-sky-300 font-medium mb-4">申请已提交，审核通过后显示</p>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. Hyakuyaの小站" value={name} onChange={e => setName(e.target.value)}
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. https://hyakuya.cc.cd" value={url} onChange={e => setUrl(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent transition-all text-sm" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">
                  ...
                  <textarea placeholder="e.g. 个人网站，分享自己喜欢和感兴趣的东西" value={description} onChange={e => setDescription(e.target.value)} rows={3}
                ...
                <div>
                  <label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">头像链接</label>
                  <input type="text" placeholder="e.g. https://github.com/xxx.png" value={avatar} onChange={e => setAvatar(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent transition-all text-sm" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">
                  ...
                  <input type="text" placeholder="e.g. hyakuyasite@gmail.com" value={email} onChange={e => setEmail(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent transition-all text-sm" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">标签</label>
                  <input type="text" placeholder="e.g. 技术，Java，生活，游戏" value={tags} onChange={e => setTags(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent transition-all text-sm" />
                </div>
